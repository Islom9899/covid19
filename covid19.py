...
uploaded_recovered = st.file_uploader('확복자 CSV 파일 업로드',type=['csv'])

#  모든 파일이 업로드 되었다면 실행
if uploaded_confirmed and uploaded_deaths and uploaded_recovered:
    # csv 파일 내용 읽어서 데이터프레임으로
    df_confirmed = pd.read_csv(uploaded_confirmed) # 확진자 -df
    df_deaths = pd.read_csv(uploaded_deaths) #사망자-  df
    df_recovered = pd.read_csv(uploaded_recovered) #확복자 df
    ...
    # 일일 증가량 계산 (오늘 - 어제)
    # df_merged['확진자'] --> 눅적(날짜별로 계속 증가하는 수치)
    # .diff() -> 차이(오늘 -어제)
    # .fillna(0) --> brinchi kun yechish yoq nan -> 0으로 채운다
    # 일일 증가량 계산
    ...
    with tab3:
        ...
        # 조건 연산자 -> 결과를 담는 변수 = 참일 경우 삾 if 조건식 else 거짓일 경우 값
        # 회복률 = (회복자 / 확진자) *100
